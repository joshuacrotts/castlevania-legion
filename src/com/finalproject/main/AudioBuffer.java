...
/**
 * This class will be an abstract class that will have references to a bunch of
 * StandardAudio buffer arrays that many classes will call; it's so multiple
 * audio files can be called at the same time and not have them interlace each
 * other/overlap/cut each other off.
 *
 * The buffers will automatically choose the most available sfx in the array,
 * and play it. When the sfx is over, it will be turned "off", and available to
 * use.
 *
 * @author Joshua
 *
